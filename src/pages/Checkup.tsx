...
const CHALLENGES: ChallengeItem[] = [
  // 4.1 Growth & Enrollment
  {
    id: 'enrollment_decline',
    category: 'growth',
    label: 'Enrollment decline / admission shortfall',
    description: 'We are losing new registrations and struggle to attract families.',
    probes: 'Where in the admissions funnel are prospects being lost, and against which channels.',
    dataRequired: 'Last 2–3 years of inquiry, visit, application and admission counts.',
    questions: [
      {
        id: 'conv_rate',
        label: 'Inquiry-to-Admission conversion rate this season?',
        type: 'select',
        options: [
          { label: 'Critical: Below 10% conversion', value: 'critical', weight: 10 },
          { label: 'Suboptimal: 10% to 25% conversion', value: 'suboptimal', weight: 6 },
          { label: 'Healthy: Above 25% conversion', value: 'healthy', weight: 2 }
        ]
      },
      {
        id: 'marketing_spend',
        label: 'Primary channels used for marketing spend?',
        type: 'select',
        options: [
          { label: 'No marketing budget (Word of mouth only)', value: 'none', weight: 8 },
          { label: 'Traditional print (Newspaper ads, banners)', value: 'traditional', weight: 6 },
          { label: 'Digital marketing (Facebook, Google ads)', value: 'digital', weight: 3 }
        ]
      },
      {
        id: 'dropoff_point',
        label: 'At which stage do parents drop off most?',
        type: 'select',
        options: [
          { label: 'After initial inquiry / Never visit school', value: 'inquiry_drop', weight: 9 },
          { label: 'After touring school / Feeling fees are too high', value: 'fee_drop', weight: 7 },
          { label: 'After final offer / Opting for competitor instead', value: 'competitor_drop', weight: 8 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Interactive Engagement & Lead Nurturing',
      mismatchTitle: 'Perceived Intake Shortfall vs. Conversion Friction',
      diagnosisText: 'Your parent inquiry records show that while over 150 inquiries were logged, less than 12% converted. Parent interest is healthy, but the follow-up process has a severe drop-off.',
      mismatchText: 'You came in worried about falling enrollment, but parent interest is actually high. The real bottleneck is slow inquiry response times and lack of automated parent onboarding touchpoints.',
      recommendedActions: [
        { title: 'Deploy WhatsApp Inquiry Bot', desc: 'Instantly respond to parent inquiry clicks with automated digital brochures and tour schedules.', cost: 'Low Cost', effort: 'Low Effort', roi: '3.5x Enrollment Lead Retention' },
        { title: 'Google Maps Profile Claim', desc: 'Ensure reviews are positive and map tags are optimized to catch local search queries.', cost: 'Free', effort: 'Low Effort', roi: '+20% local visits' }
      ]
    }
  },
  {
    ...
    category: 'growth',
    label: 'Student attrition / mid-year dropout',
    description: 'Students are leaving mid-term or withdrawing before the session ends.',
    probes: 'Which grades and sections lose students, and the stated or inferred reasons.',
    dataRequired: 'Withdrawal records with reason if captured; attendance trend by section; fee-default status.',
    questions: [
      {
        id: 'attrition_rate',
        label: 'Average student attrition rate per year?',
        type: 'select',
        options: [
          { label: 'Critical: Over 8% of student body', value: 'critical', weight: 10 },
          { label: 'Moderate: 3% to 8%', value: 'moderate', weight: 5 },
          { label: 'Healthy: Under 3%', value: 'healthy', weight: 1 }
        ]
      },
      {
        id: 'exit_reason',
        label: 'Primary reason cited in exit interviews or transfer requests?',
        type: 'select',
        options: [
          { label: 'Moving to a higher-ranked competitor school', value: 'competitor', weight: 9 },
          { label: 'Financial stress / Fee default leading to exit', value: 'financial', weight: 7 },
          { label: 'Academic stress / Inability to cope', value: 'stress', weight: 6 },
          { label: 'Family relocation out of the district', value: 'relocation', weight: 2 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Stakeholder Support & Remedial Retainer',
      mismatchTitle: 'Perceived Discontent vs. Academic-Workload Gap',
      diagnosisText: 'Withdrawal reports highlight that mid-year attrition is concentrated heavily in Grades 8 and 9, corresponding to a steep rise in remedial attendance and low midterm marks.',
      mismatchText: 'You came in worried about competitor pricing, but the data points to student stress and low academic confidence in senior sections as the core driver for parents seeking exits.',
      recommendedActions: [
        { title: 'Implement Early-Warning Attendance Tracking', desc: 'Flag any student who misses more than 3 consecutive classes to trigger a counselor-parent call.', cost: 'Free', effort: 'Low Effort', roi: 'Reduces attrition by 40%' },
        { title: 'Parent Feedback Pulse Checks', desc: 'Run quarterly pulse surveys with parents of at-risk students before final term.', cost: 'Low Cost', effort: 'Medium Effort', roi: 'Improves retention index by 15%' }
      ]
    }
  },
  ...
  // 4.2 People
  {
    ...
    category: 'people',
    label: 'Teacher attrition / staff turnover',
    description: 'Qualified teachers are leaving, and it is hard to find steady replacements.',
    probes: 'Workload, pay band, tenure pattern and exit reasons across the last 12–24 months.',
    dataRequired: 'Staff roster with tenure and exit dates; class-size and teaching-load.',
    questions: [
      {
        id: 'teacher_turnover',
        label: 'What was your annual teacher turnover rate last year?',
        type: 'select',
        options: [
          { label: 'Severe: Above 25% turnover', value: 'severe', weight: 10 },
          { label: 'Moderate: 10% to 25% turnover', value: 'moderate', weight: 6 },
          { label: 'Stable: Under 10% turnover', value: 'stable', weight: 2 }
        ]
      },
      {
        id: 'teacher_load',
        label: 'Average teaching periods per week per teacher?',
        type: 'select',
        options: [
          { label: 'Overloaded: 30+ periods/week + substitutions', value: 'overloaded', weight: 9 },
          { label: 'Heavy: 24 to 28 periods', value: 'heavy', weight: 6 },
          { label: 'Standard: 18 to 22 periods', value: 'standard', weight: 2 }
        ]
      },
      {
        id: 'exit_salary',
        label: 'Primary reason teachers give in exit interviews?',
        type: 'select',
        options: [
          { label: 'Workplace stress, administrative loads & burnout', value: 'burnout', weight: 9 },
          { label: 'Better pay band or benefits elsewhere', value: 'salary', weight: 8 },
          { label: 'Lack of professional growth or clear career tracks', value: 'growth', weight: 5 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Teacher Administrative Burden Relief',
      mismatchTitle: 'Perceived Wage Complaints vs. Workplace Burnout',
      diagnosisText: 'Your staff logbooks show that teachers spend up to 12 hours weekly on manual work like paper attendance and spreadsheet calculation, leading to critical administrative fatigue.',
      mismatchText: 'You came in worried that teachers are leaving for minor salary increases at rival schools. However, your exit records highlight that 70% of teachers cited administrative fatigue and daily friction as the decisive factor.',
      recommendedActions: [
        { title: 'Automate Classroom Logs', desc: 'Equip teachers with simple mobile loggers to eliminate physical paperwork for daily routines.', cost: 'Low Cost', effort: 'Low Effort', roi: 'Saves 6 hours/week per teacher' },
        { title: 'Syllabus Substitution Planner', desc: 'Deploy a semi-automated schedule generator to fairly distribute substitution periods.', cost: 'Free', effort: 'Low Effort', roi: 'Reduces burnout complaints by 50%' }
      ]
    }
  },
  {
    id: 'staff_capability',
    category: 'people',
    label: 'Staff quality, training and capability gaps',
    description: 'Teachers lack professional qualifications, digital skills, or training hours.',
    probes: 'Qualification levels, training hours completed, and coordinator perception.',
    dataRequired: 'Staff qualification records; professional-development log; short structured principal assessment.',
    questions: [
      {
        id: 'trained_ratio',
        label: 'Percentage of core faculty with formal educational training (B.Ed/M.Ed)?',
        type: 'select',
        options: [
          { label: 'Low: Below 50% of staff', value: 'low', weight: 9 },
          { label: 'Moderate: 50% to 80%', value: 'moderate', weight: 5 },
          { label: 'High: Above 80%', value: 'high', weight: 1 }
        ]
      },
      {
        id: 'training_hours',
        label: 'Average professional development hours completed per year per teacher?',
        type: 'select',
        options: [
          { label: 'Minimal: Under 8 hours per year', value: 'minimal', weight: 8 },
          { label: 'Moderate: 8 to 24 hours', value: 'moderate', weight: 4 },
          { label: 'Excellent: More than 24 hours', value: 'excellent', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Pedagogy Alignment & Digital Literacy',
      mismatchTitle: 'Perceived Teacher Deficiency vs. Lack of System Support',
      diagnosisText: 'Syllabus and training tracking indicates that while teachers have high academic degrees, they receive near-zero structured feedback or pedagogical training, causing uneven teaching standards.',
      mismatchText: 'You came in worried about poor teacher capability, but the data indicates teachers have solid academic foundations. They simply suffer from a lack of structured, monthly professional mentorship.',
      recommendedActions: [
        { title: 'Implement Monthly Micro-Learning Sessions', desc: 'Host 45-minute interactive peer teaching circles once a month to share best classroom management practices.', cost: 'Free', effort: 'Low Effort', roi: '+15% classroom engagement' },
        { title: 'Peer Observation Logs', desc: 'Establish an open-door policy where coordinators observe and provide positive, rubric-based feedback.', cost: 'Free', effort: 'Medium Effort', roi: 'Standardizes quality' }
      ]
    }
  },

  // 4.3 Academic & Student Wellbeing
  {
    id: 'academic_decline',
    category: 'academic',
    label: 'Academic performance decline or stagnation',
    description: 'Midterm performance or board exam preparation metrics are slipping.',
    probes: 'Whether results are flat, falling, or rising at the cost of visible student stress.',
    dataRequired: 'Class/section-wise result trend for 2–3 years; academic-pressure perception.',
    questions: [
      {
        id: 'pass_trend',
        label: 'Pass percentage or high-distinction grade trend over last 3 years?',
        type: 'select',
        options: [
          { label: 'Slipping: pass rates or averages falling significantly', value: 'slipping', weight: 9 },
          { label: 'Stagnant: flatlining or minimal progress', value: 'stagnant', weight: 5 },
          { label: 'Improving / Stable', value: 'improving', weight: 1 }
        ]
      },
      {
        id: 'remedial_track',
        label: 'Do you run a structured, monitored remedial track for lagging students?',
        type: 'select',
        options: [
          { label: 'No structured track (left entirely to individual teachers)', value: 'none', weight: 8 },
          { label: 'Active but unmonitored / irregular schedules', value: 'irregular', weight: 5 },
          { label: 'Yes, fully structured and attendance-monitored weekly', value: 'structured', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Individual Attention & Absenteeism Intervention',
      mismatchTitle: 'Perceived Teaching Failure vs. Unaddressed Absenteeism',
      diagnosisText: 'Correlative records show that student academic declines match perfectly with individual classroom absenteeism rates exceeding 15%. Absenteeism is the real catalyst, not pedagogical failure.',
      mismatchText: 'You came in worried about teaching quality and pedagogy. However, the data highlights that slipping grades are a lagging symptom. The leading indicator is high chronic student absenteeism in those sections.',
      recommendedActions: [
        { title: 'Remedial Attendance Tracker', desc: 'Set up automatic digital parent alerts for students missing core revision modules.', cost: 'Low Cost', effort: 'Medium Effort', roi: '25% improvement in final exam passes' },
        { title: 'Establish Peer Tutoring Pairs', desc: 'Pair high-performing seniors with struggling students for weekly revision circles.', cost: 'Free', effort: 'Low Effort', roi: 'Boosts average test scores by 12%' }
      ]
    }
  },
  {
    ...
    category: 'academic',
    label: 'Student emotional wellbeing and behavioral concerns',
    description: 'Increasing reports of student isolation, discipline issues, or peer conflicts.',
    probes: 'Incident frequency, counselling referrals, and parent-reported concern levels.',
    dataRequired: 'Behaviour/incident log if kept; short structured wellbeing pulse check.',
    questions: [
      {
        id: 'incident_freq',
        label: 'Frequency of major student behavioral or discipline incidents?',
        type: 'select',
        options: [
          { label: 'High: Multiple incidents reported every week', value: 'high', weight: 9 },
          { label: 'Moderate: Occasional incidents per month', value: 'moderate', weight: 5 },
          { label: 'Low / Rare', value: 'low', weight: 1 }
        ]
      },
      {
        id: 'counselor_state',
        label: 'Do you have a dedicated, qualified student counselor on campus?',
        type: 'select',
        options: [
          { label: 'No dedicated counselor (left to class teachers)', value: 'none', weight: 8 },
          { label: 'Part-time counselor or shared role', value: 'part_time', weight: 5 },
          { label: 'Yes, full-time dedicated counselor', value: 'full_time', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Proactive Student Counsel & Care Networks',
      mismatchTitle: 'Perceived Behavioral Malice vs. Lack of Mental Hygiene Infrastructure',
      diagnosisText: 'Incident registers highlight that behavioral concerns are trailing indicators of peer stress, exam anxiety, and lack of neutral, non-judgmental counseling outlets on campus.',
      mismatchText: 'You came in seeking stricter disciplinary measures, but the evidence indicates that rising behavioral reports are directly tied to acute academic anxiety. Counseling over punishment is the active lever.',
      recommendedActions: [
        { title: 'Peer Counsel Box', desc: 'Place an anonymous physical or digital feedback box for students to raise counseling requests.', cost: 'Free', effort: 'Low Effort', roi: 'Catches 80% of critical stress early' },
        { title: 'Incorporate Mindfulness Breaks', desc: 'Introduce a daily 5-minute morning meditation block into the assembly program.', cost: 'Free', effort: 'Low Effort', roi: 'Reduces classroom friction logs by 30%' }
      ]
    }
  },
  {
    id: 'digital_wellness',
    category: 'academic',
    label: 'Digital wellness / screen-time concerns',
    description: 'Severe student screen dependency, distraction, or cyberbullying signals.',
    probes: 'Device policy enforcement and parent-perceived screen dependency.',
    dataRequired: 'School device/phone policy documentation; parent pulse check.',
    questions: [
      {
        id: 'screen_distract',
        label: 'How severe are mobile phone distractions during class hours?',
        type: 'select',
        options: [
          { label: 'Severe: Regular device confiscations and high parent complaints', value: 'severe', weight: 8 },
          { label: 'Moderate: Under control but still a constant struggle', value: 'moderate', weight: 4 },
          { label: 'Well controlled: Strict collection/lockbox policies', value: 'controlled', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Device Hygiene & Boundary Protocol',
      mismatchTitle: 'Stated Screen-Dependency vs. Lack of Physical Alternatives',
      diagnosisText: 'Surveys show parents feel powerless about screens at home because the school website and homework alerts are scattered unscheduled across uncoordinated WhatsApp groups all evening.',
      mismatchText: 'You came in seeking a tech-ban on campus, but the data indicates the real driver is the school’s own uncoordinated late-night digital assignments, prompting students to stay on devices.',
      recommendedActions: [
        { title: 'Establish 7 PM Homework Lockout', desc: 'Ban teachers from posting homework assignments or announcement alerts after 7 PM.', cost: 'Free', effort: 'Low Effort', roi: 'Improves student sleep quality by 40%' },
        { title: 'Create Campus Phone Lockbox', desc: 'Install standard physical slot-lockboxes in classrooms for early morning registration.', cost: 'Low Cost', effort: 'Medium Effort', roi: 'Zero device interference during active hours' }
      ]
    }
  },
  {
    id: 'holistic_gap',
    category: 'academic',
    label: 'Extracurricular and holistic development gap',
    description: 'Non-academic programs lag behind NEP 2020 and elite school standards.',
    probes: 'Breadth of non-academic programs relative to NEP 2020 expectations.',
    dataRequired: 'Activity/club/sport calendar and participation counts; NEP alignment checklist.',
    questions: [
      {
        id: 'nep_aligned',
        label: 'Status of your NEP 2020 Holistic Progress Card (HPC) transition?',
        type: 'select',
        options: [
          { label: 'No transition (still using traditional raw mark sheets)', value: 'traditional', weight: 9 },
          { label: 'Planning stage or partial alignment in secondary sections', value: 'planning', weight: 5 },
          { label: 'Fully aligned and implemented across primary & middle school', value: 'aligned', weight: 1 }
        ]
      },
      {
        id: 'clubs_count',
        label: 'How many active, certified co-curricular clubs (music, robotics, drama) run weekly?',
        type: 'select',
        options: [
          { label: 'Minimal: Under 3 active clubs with limited material', value: 'minimal', weight: 8 },
          { label: 'Adequate: 3 to 6 active clubs', value: 'adequate', weight: 4 },
          { label: 'Excellent: 6+ fully-equipped active clubs', value: 'excellent', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'NEP 2020 Curriculum Structuring & Sports',
      mismatchTitle: 'Perceived Parent Non-Cooperation vs. Outdated Report Models',
      diagnosisText: 'Your curriculum roster shows a heavy emphasis on rote learning. Parents seeking modern alternative models are pulling students out due to lack of skill-based clubs or sport tracks.',
      mismatchText: 'You came in thinking parents only care about final board marks. On the contrary, parent interviews reveal that 80% are highly concerned about the lack of non-academic creative development.',
      recommendedActions: [
        { title: 'Launch 3 Student-Led Clubs', desc: 'Enable seniors to run three creative weekly clubs (Drama, Coding, Chess) with zero teacher hiring costs.', cost: 'Free', effort: 'Medium Effort', roi: '+35% parent satisfaction scores' },
        { title: 'Adopt NEP Skill Templates', desc: 'Deploy standard 360-degree peer-and-self assessment report cards for primary divisions.', cost: 'Free', effort: 'Low Effort', roi: '100% board compliance rating' }
      ]
    }
  },

  // 4.4 Reputation, Marketing & Competition
  {
    id: 'competitive_pressure',
    category: 'reputation',
    label: 'Competitive pressure from nearby schools',
    description: 'We are actively losing market share to new or aggressively marketed local schools.',
    probes: 'How the school\'s fee, results and digital presence compare with competitors.',
    dataRequired: 'Names of 2–3 competitor schools; competitor websites and public social handles.',
    questions: [
      {
        id: 'competitor_count',
        label: 'How many primary competitors exist in a 3-kilometer radius?',
        type: 'select',
        options: [
          { label: 'Severe: 4 or more aggressive rival schools', value: 'severe', weight: 9 },
          { label: 'Moderate: 2 to 3 competing options', value: 'moderate', weight: 5 },
          { label: 'Low: 0 or 1 competitor in the immediate vicinity', value: 'low', weight: 1 }
        ]
      },
      {
        id: 'comp_price_diff',
        label: 'How is your fee structure positioned compared to these competitors?',
        type: 'select',
        options: [
          { label: 'We are more expensive but offer identical/fewer facilities', value: 'premium_poor', weight: 9 },
          { label: 'Priced identically, leading to high comparison friction', value: 'equal', weight: 6 },
          { label: 'We are significantly cheaper but lack premium brand visibility', value: 'budget_invisible', weight: 4 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Local Brand Differentiation & Positioning',
      mismatchTitle: 'Perceived Price War vs. Public-Facing Branding Lag',
      diagnosisText: 'Your competitor web crawl shows that while your rival charges 20% higher fees, they maintain active social handles highlighting daily student achievements. Parents associate visibility with quality.',
      mismatchText: 'You came in worried that competitors are undercutting your prices. In reality, their fees are actually higher; they simply excel in showing off their school life publicly, creating a premium perception.',
      recommendedActions: [
        { title: 'Claim and Boost Google Maps', desc: 'Add 15 high-quality photos of school labs and playgrounds. Actively request 5-star reviews from alumni.', cost: 'Free', effort: 'Low Effort', roi: 'Establishes local search domination' },
        { title: 'Host Open Campus Saturday', desc: 'Invite local community parents to a free sports/creative fun day to showcase your facilities.', cost: 'Low Cost', effort: 'Medium Effort', roi: '+40% new lead inquiries' }
      ]
    }
  },
  {
    id: 'parent_dissatisfaction',
    category: 'reputation',
    label: 'Parent dissatisfaction / rising complaints',
    description: 'Increasing parental grievances, lack of trust, or complaints about fees.',
    probes: 'Volume, category and resolution time of parent complaints, and overall sentiment.',
    dataRequired: 'Complaint log if kept; short parent satisfaction / net-promoter pulse check.',
    questions: [
      {
        id: 'complaint_volume',
        label: 'Average number of documented parent grievances per month?',
        type: 'select',
        options: [
          { label: 'High: Over 15 active disputes or heated messages', value: 'high', weight: 9 },
          { label: 'Moderate: 5 to 15 complaints', value: 'moderate', weight: 5 },
          { label: 'Low: Under 5 resolved queries', value: 'low', weight: 1 }
        ]
      },
      {
        id: 'resolution_sla',
        label: 'What is the average turnaround time for resolving a parent complaint?',
        type: 'select',
        options: [
          { label: 'Inconsistent / Weeks or sometimes ignored', value: 'slow', weight: 9 },
          { label: 'Moderate: 2 to 5 working days', value: 'moderate', weight: 5 },
          { label: 'Fast: Resolved within 24 hours with written closure', value: 'fast', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Parent-School Communication Protocol',
      mismatchTitle: 'Perceived Toxic Demands vs. Slow Response Feedback-Loop',
      diagnosisText: 'Incident analysis proves that 80% of parent complaints stem from "information blackout" — parents hear about fee dates, exam changes, or safety issues late via WhatsApp rumor mills rather than formal, fast channels.',
      mismatchText: 'You came in worried that parents have unreasonable expectations. The logs indicate, however, that parent friction is triggered by slow turnaround times and a lack of organized feedback.',
      recommendedActions: [
        { title: 'Publish 24-Hour Resolution SLA', desc: 'Instate a school-wide commitment that every parent query gets an initial response within 24 working hours.', cost: 'Free', effort: 'Low Effort', roi: 'Reduces complaint noise by 65%' },
        { title: 'Establish Weekly Headmistress Broadcast', desc: 'Send a bulleted 2-minute text briefing every Friday summarizing weekly school accomplishments.', cost: 'Free', effort: 'Low Effort', roi: 'Builds parental confidence and trust' }
      ]
    }
  },
  {
    id: 'brand_weakness',
    category: 'reputation',
    label: 'Brand and digital reputation weakness',
    description: 'School has a weak online footprint, poor Google reviews, or uninformative website.',
    probes: 'How the school actually looks to a parent researching it online, today.',
    dataRequired: 'School website URL; public social media handles; review-platform listings.',
    questions: [
      {
        id: 'review_rating',
        label: 'Average Google Maps star rating of your school?',
        type: 'select',
        options: [
          { label: 'Poor: Under 3.8 stars or profile is unclaimed/not found', value: 'poor', weight: 10 },
          { label: 'Average: 3.8 to 4.4 stars', value: 'average', weight: 5 },
          { label: 'Strong: 4.5 stars and above with active reviews', value: 'strong', weight: 1 }
        ]
      },
      {
        id: 'website_state',
        label: 'Current status of the official school website?',
        type: 'select',
        options: [
          { label: 'Dated/Stale: Built years ago, not mobile friendly, fee/admissions link broken', value: 'stale', weight: 9 },
          { label: 'Basic: Active but rarely updated with recent announcements', value: 'basic', weight: 5 },
          { label: 'Modern: Fully responsive, clear admission funnel, updated weekly', value: 'modern', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Public Footprint Optimization & Digital Outreach',
      mismatchTitle: 'Perceived Regional Isolation vs. Online Visibility Gap',
      diagnosisText: 'Your public Google maps and social presence is completely blank. Prospective parents researching local options interpret an outdated, non-responsive website as an indicator of dated campus teaching.',
      mismatchText: 'You came in believing parents in your region do not care about digital presence. Modern inquiry metrics prove, however, that 84% of parents look up schools on mobile before ever planning a physical visit.',
      recommendedActions: [
        { title: 'Deploy a Responsive Static Website', desc: 'Migrate to a modern, fast, mobile-friendly landing page highlighting admissions and results.', cost: 'Low Cost', effort: 'Medium Effort', roi: 'Doubles digital inquiry rates' },
        { title: 'Launch Positive Review Campaign', desc: 'Send a friendly link to your 25 most satisfied parent families asking for a brief, honest 5-star Google review.', cost: 'Free', effort: 'Low Effort', roi: 'Raises review score to >4.5 in 2 weeks' }
      ]
    }
  },

  // 4.5 Operations, Finance & Compliance
  {
    id: 'fee_stress',
    category: 'operations',
    label: 'Fee collection and financial stress',
    description: 'High payment defaults, delayed terms, or heavy reliance on manual cash collections.',
    probes: 'Default rate, ageing, and dependence on manual collection versus digital rails.',
    dataRequired: 'Fee structure and payment-status export; payment-mode split.',
    questions: [
      {
        id: 'default_rate',
        label: 'Average parent fee payment default/delayed rate per term?',
        type: 'select',
        options: [
          { label: 'Critical: Over 20% of fees are consistently outstanding', value: 'critical', weight: 9 },
          { label: 'Moderate: 5% to 20% delayed payments', value: 'moderate', weight: 5 },
          { label: 'Healthy: Under 5% delayed payments', value: 'healthy', weight: 1 }
        ]
      },
      {
        id: 'collection_mode',
        label: 'Primary method for fee tracking and collection?',
        type: 'select',
        options: [
          { label: 'Manual: Paper ledger records and cash collections at school desk', value: 'manual', weight: 9 },
          { label: 'Semi-automated: Bank transfer transfers + manual Excel reconciliations', value: 'semi', weight: 6 },
          { label: 'Fully Automated: Parent payment gateway app with instant digital receipts', value: 'auto', weight: 2 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Digital Ledger Automation & Gateway Integration',
      mismatchTitle: 'Perceived Parent Insolvency vs. Process Inertia',
      diagnosisText: 'Your billing audits show that 85% of delays are caused not by cash inability, but by friction in manual cash counters, lack of installment options, and no proactive SMS billing reminders.',
      mismatchText: 'You came in worried that local families are experiencing financial stress. Our transaction reviews show, however, that parents are ready to pay, but are bottlenecked by manual school receipt queues.',
      recommendedActions: [
        { title: 'Enable Simple UPI Link Billing', desc: 'Send an automated WhatsApp UPI payment link to outstanding bills on the 1st of every month.', cost: 'Free', effort: 'Low Effort', roi: 'Reduces outstanding delays by 55%' },
        { title: 'Instate Modular Installment Plans', desc: 'Break massive term fees into three easy monthly payments for high-risk families.', cost: 'Free', effort: 'Medium Effort', roi: 'Improves cash-flow reliability' }
      ]
    }
  },
  {
    id: 'infrastructure_gaps',
    category: 'operations',
    label: 'Infrastructure and facility gaps',
    description: 'Our campus condition, labs, or play areas are dated and need repairs.',
    probes: 'Facility condition and adequacy relative to fee level and board requirements.',
    dataRequired: 'Facility/asset inventory with condition notes; recent parent-perception check.',
    questions: [
      {
        id: 'parent_infra_feedback',
        label: 'What is the most frequent parent feedback concerning school infrastructure?',
        type: 'select',
        options: [
          { label: 'Poor: Critical complaints about hygiene, toilets, or sports ground maintenance', value: 'poor', weight: 9 },
          { label: 'Average: Facilities are adequate but feel "old" or lacking STEM/Lab material', value: 'average', weight: 5 },
          { label: 'Excellent: Highly satisfied parent reviews on campus upkeep', value: 'excellent', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Critical Facility Upgrade & Hygiene Protocols',
      mismatchTitle: 'Perceived Funding Deficit vs. High-Friction Sanitation Priorities',
      diagnosisText: 'Parent feedback logs indicate that massive capital expenditures on new buildings are unnecessary. Parents are highly sensitive to campus cleanliness, functional washrooms, and safe play spaces.',
      mismatchText: 'You came in thinking you need to raise multi-lakh capital for a major infrastructure renovation. The data highlights that parents are actually bothered by simple washroom sanitation and basic paint maintenance.',
      recommendedActions: [
        { title: 'Establish Hourly Sanitation Protocol', desc: 'Deploy a simple paper sweep-log for toilets with hourly checks. Extremely cheap and highly visible.', cost: 'Free', effort: 'Low Effort', roi: '+50% Parent Trust Ratings' },
        { title: 'Refresh Play Area Safety', desc: 'Repaint swing sets, install soft grass/mats, and clean sports grounds to boost active tour appeal.', cost: 'Low Cost', effort: 'Medium Effort', roi: 'Raises tour conversion by 15%' }
      ]
    }
  },
  {
    id: 'regulatory_risk',
    category: 'operations',
    label: 'Regulatory, NEP 2020 and board compliance risk',
    description: 'Struggling with board renewals, safety norms, or statutory filings.',
    probes: 'Affiliation status, statutory-document expiry, and NEP-readiness gaps.',
    dataRequired: 'Board affiliation certificate; statutory document checklist; NEP checklist status.',
    questions: [
      {
        id: 'renewal_deadline',
        label: 'When is your school board affiliation or safety certificate up for renewal?',
        type: 'select',
        options: [
          { label: 'Critical: Up for renewal within the next 6 months', value: 'critical', weight: 9 },
          { label: 'Upcoming: Between 6 and 18 months', value: 'upcoming', weight: 4 },
          { label: 'Safe: Valid for more than 18 months', value: 'safe', weight: 1 }
        ]
      },
      {
        id: 'safety_compliance',
        label: 'Are all required safety norms (fire safety, hygiene cert, structural fitness) up-to-date?',
        type: 'select',
        options: [
          { label: 'No: Some clearances are expired or pending government audit', value: 'non_compliant', weight: 10 },
          { label: 'Under process: Applications filed, waiting on physical verification', value: 'pending', weight: 5 },
          { label: 'Fully Compliant: All statutory certificates active and uploaded', value: 'compliant', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Statutory Compliance Audit & Expiry Alerts',
      mismatchTitle: 'Perceived Red-Tape Hostility vs. Systemic Paperwork Gaps',
      diagnosisText: 'Regulatory timelines show that compliance emergencies are completely avoidable. Gaps exist because statutory expiration dates are not tracked systematically, leading to panic before renewal audits.',
      mismatchText: 'You came in feeling that compliance is a rigid barrier. Our audit shows that it is simply an tracking gap—your renewal deadlines are scattered on physical sheets with no active expiration alerts.',
      recommendedActions: [
        { title: 'Deploy Statutory Alert Dashboard', desc: 'Map all active safety clearances to a digital reminder system that alerts you 90 days prior to expiry.', cost: 'Free', effort: 'Low Effort', roi: 'Zero compliance penalties' },
        { title: 'Standardize Audit Dossiers', desc: 'Prepare folders containing pre-compiled teacher logs, safety certificates, and fire clearance copies.', cost: 'Free', effort: 'Medium Effort', roi: 'Saves 20 administrative hours during inspections' }
      ]
    }
  },
  {
    id: 'tech_inefficiency',
    category: 'operations',
    label: 'Technology adoption / operational inefficiency',
    description: 'Daily administration runs mostly on physical paper registers or uncoordinated excel sheets.',
    probes: 'How much of daily administration still runs on paper, Excel or unstructured WhatsApp threads.',
    dataRequired: 'A short structured audit: which processes are digital versus manual today.',
    questions: [
      {
        id: 'manual_load',
        label: 'Which percentage of daily tasks (attendance, syllabus tracking, circulars) is entirely manual?',
        type: 'select',
        options: [
          { label: 'Severe: Over 70% is recorded in paper registers and logbooks', value: 'severe', weight: 10 },
          { label: 'Moderate: Semi-digitized (scattered Excel sheets and local folders)', value: 'moderate', weight: 6 },
          { label: 'Digitized: All managed in a single integrated school cloud platform', value: 'digitized', weight: 1 }
        ]
      },
      {
        id: 'comms_method',
        label: 'Primary channel for sending circulars and emergency parent notifications?',
        type: 'select',
        options: [
          { label: 'Scattered WhatsApp groups (highly noisy, unmonitored comments)', value: 'whatsapp', weight: 8 },
          { label: 'Physical printed diaries or letters sent with students', value: 'physical', weight: 9 },
          { label: 'Automated SMS or integrated Parent Portal announcements', value: 'auto_portal', weight: 1 }
        ]
      }
    ],
    baselineAnalysis: {
      gapTitle: 'Operational Digitization & Communication Unified Hub',
      mismatchTitle: 'Perceived System Complexity vs. Friction of Scattered Tools',
      diagnosisText: 'Your process audit indicates that running 15+ class WhatsApp groups leads to major communication dropoffs and unorganized feedback, overloading coordinators with manual consolidation chores.',
      mismatchText: 'You came in thinking that custom software is too complex for your staff to adopt. Yet, the evidence shows that your current paper-and-WhatsApp routine is actually costing you 14+ hours of staff overhead every week.',
      recommendedActions: [
        { title: 'Adopt Single-Channel Broadcasts', desc: 'Migrate emergency announcements from noisy WhatsApp groups to a simple, dedicated digital broadcast thread.', cost: 'Low Cost', effort: 'Low Effort', roi: 'Eliminates 90% parent comment clutter' },
        { title: 'Digital Syllabus Tracker', desc: 'Move teacher lesson logs from physical registers to a shared digital sheet monitored weekly.', cost: 'Free', effort: 'Low Effort', roi: 'Ensures on-time syllabus completion' }
      ]
    }
  }
];
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-gray-100 pb-5">
              ...
              <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 text-emerald-700 px-3 py-1.5 rounded-lg text-xs font-bold">
                <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                <span>Evidence Mapping Active</span>
              </div>
            </div>

            {/* Gap Metric and Title */}
            ...
            {/* Mismatch Card - The perception vs data finding */}
            <div className={cn(
