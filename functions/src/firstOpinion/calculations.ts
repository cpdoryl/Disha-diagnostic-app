...
export interface QuestionResponse {
  text: string
  selectedOption: number // 1-10 ordinal scale
  maxOption: number // denominator for severity calculation
  isFact: boolean // Refinement 4: Fact vs Perception tagging
  factSource?: string // If isFact, which system provided data
}
...
export interface ChallengeResponse {
  ...
  schoolId: string
  cycleId: string
  responses: Record<string, QuestionResponse> // q1, q2, q3, etc.
  challenge: {
    title: string
    domain: string
...
export function calculateSsub(
  ...
  const byChallenge: Record<string, ChallengeResponse[]> = {}
...
export function calculateHealthIndex(
  s_sub: number,
  m_obj: number
): { healthIndex: number; delusionPenalty: number } {
...
export function getHealthStatus(
  healthIndex: number
): { status: string; color: string; description: string; icon: string } {
  if (healthIndex >= 80) {
    return {
      status: 'EXCELLENT',
      color: 'green',
...
export function calculateGapAndQuadrant(
  ...
  let quadrant: 'REALITY_BETTER' | 'ALIGNED' | 'PERCEPTION_BETTER'
...
export function validateChallengeResponses(
  ...
  responses.forEach((resp, idx) => {
    ...
    if (!resp.responderId) errors.push(`Response ${idx}: Missing responderId`)
    if (!resp.role) errors.push(`Response ${idx}: Missing respondent role`)
    if (!resp.email) errors.push(`Response ${idx}: Missing respondent email`)
    if (!resp.challengeId) errors.push(`Response ${idx}: Missing challengeId`)
    ...
    Object.entries(resp.responses).forEach(([qKey, q]) => {
      ...
      if (q.isFact) {
        ...
        if (!q.factSource) {
          warnings.push(
            `Response ${idx}, ${qKey}: Fact-based answer missing data source`
          )
        }
      } else {
        perceptionCount++
      ...
      if (q.selectedOption < 1 || q.selectedOption > q.maxOption) {
        errors.push(
          `Response ${idx}, ${qKey}: Invalid option ${q.selectedOption} (range: 1-${q.maxOption})`
        )
      }

      // Validate required question fields
      if (!q.text) errors.push(`Response ${idx}, ${qKey}: Missing question text`)
      if (q.maxOption < 1) errors.push(`Response ${idx}, ${qKey}: Invalid maxOption`)
    })
  })
 ...
 */
export const FirstOpinionCalculations = {
  calculateSsub,
  calculateMobj,
