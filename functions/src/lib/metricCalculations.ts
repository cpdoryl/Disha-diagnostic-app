...
export const calculateMetric1d = (data: {
  ...
  const growthValues = linkedStudents.map(
    id => data.currentYearScores[id] - data.previousYearScores[id]
  );
...
export const calculateMetric2c = (data: {
  ...
  if (lectureBased === 0) return activityBased > 0 ? 100 : 0;
...
export const calculateMetric2e = (data: { projectsDocumented: number }): number => {
...
export const calculateMetric3b = (data: { teacherTenures: number[] }): number => {
